    ...
    def user(user_id):
        ...
        if request.method == 'PUT':
            ...
            try:
                ...
                user_to_update = User.get_or_none(user_id=user_id)

                # Valida o email recebido na requisição
                encrypted_email = cipher_suite.encrypt(user['user_email'].encode())
                if User.get_or_none((User.user_email == encrypted_email.decode()) & (User.user_id != user_id)) is not None:
                ...
                for u in decrypted_users:
                    if u['user_email'] == cipher_suite.decrypt(encrypted_email).decode() and u['user_id'] != user_to_update.user_id:
                        return "Email já cadastrado", 400
                ...
                User.update(
                    ...
                    user_email=encrypted_email.decode(),
                    user_password=encrypted_password.decode(),
                    updated_at=datetime.now()).where(User.user_id == user_id).execute()
